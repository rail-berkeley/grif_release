...
class BridgeDataset:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # the expected type spec for the serialized example_sent
        ...
        for sub_data_paths in data_paths:
            # get rid of paths that don't exist
        ...
        if train and augment:
            ...
            # this was the only way I found to fix a memory leak in tf.random.Generator
    ...
    def _add_goals(self, traj) -> tf.data.Dataset:
        ...
        if self.simple_goal:
            example = {
                ...
                "actions": traj["actions"][-1],  # this is placeholder
            }
        ...
        # get the future goal image and proprioception
        traj["goals"] = {}
        ...
        # append rewards: 0 when goal is reached, -1 otherwise
        traj["rewards"] = tf.cast(tf.where(goal_reached_mask, 0, -1), tf.int32)

        # append masks; following the new gym specification, the next Q-value
        # should be masked out only when `terminated` is True. for the
        # BridgeData format, `terminals` is typically all False because every
        # trajectory is considered truncated (the last transition is valid).
