    ...
    if isinstance(batch1, dict) or isinstance(batch1, FrozenDict):
        ...
        for k in batch1:
            if k == "inter_dataset_mask":
                A = batch1[k].shape[0]
                B = batch2[k].shape[0]
                # mask = jnp.ones((A + B, A + B))
                # mask = mask.at[:A, :A].set(batch1[k])
                # mask = mask.at[A:, A:].set(batch2[k])
...
def main(_):
    ...
    load_mapping(FLAGS.config.data_path)
    ...
    if FLAGS.config.domain_weight is not None:
        # train_paths = [x for x in train_paths if x not in scene_train_paths]
        # sample_weights = [1 - FLAGS.config.domain_weight, FLAGS.config.domain_weight]
    ...
    # probably takes less steps to converge
    ...
    def process_batch(batch, data_split):
        ...
        if "bridgedata" in data_split:
            sents = [lang_decode(x) for x in lang_ids]
        ...
        if (
            ...
            # not used when reading embeddings, placeholder
            lang_inputs = jnp.ones((len(sents), 64))

        # TODO this is actually a bottleneck; skip it if using frozen embeds
        if (
        ...
        # just use this and drop the other stuff
        processed_batch = {
    ...
    print(example_batch["goals"].keys())
    # print(example_batch["goals"]["image"][:, :3, :3, :])
    ...
    if "drop_encoders" in FLAGS.config and FLAGS.config.drop_encoders:
        # do this when using frozen embeddings
        task_encoder_defs = {
    ...
    if "clip" in FLAGS.config.task_encoders.get(
        ...
        for module_key in clip_module_vars:
            # these keys are intentionally named in jaxrl_m.vision.clip
            if module_key == "temperature":
                continue
            pretrained_params["clip_" + module_key] = clip_module_vars[module_key]
        # if "temperature" in clip_module_vars.keys():
        # pretrained_params["contrastive_temp"] = clip_module_vars["temperature"]
            ...
            for mod in FLAGS.config.task_encoders:
                for split, val_data in val_datasets.items():
                    ...
                    for batch in tqdm.tqdm(
                        ...
                        # TODO ss2 will produce nans for BC metrics because it doesn't have actions
